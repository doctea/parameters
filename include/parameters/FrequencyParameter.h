...
class FrequencyParameter : public DataParameter<TargetClass, DataType> {
    public:
        ...
        FrequencyParameter(char *label, TargetClass *target, float initial_value, void(TargetClass::*setter_func)(DataType), DataType octave_range = 5.0) 
            : DataParameter<TargetClass,DataType>(label, target, initial_value, setter_func) {
                this->octave_range = octave_range;
            };
        ...
        FrequencyParameter(char *label, TargetClass *target, void(TargetClass::*setter_func)(DataType), void(TargetClass::*getter_func)(DataType), DataType octave_range = 5.0) 
            : DataParameter<TargetClass,DataType>(label, target, setter_func, getter_func) 
            {
            this->octave_range = octave_range;
        };

        //TODO: move helper functions from ads.cpp ie get_frequency_for_voltage etc to here
        ...
        /*virtual void updateValue(DataType value) override {

            //this->octave_range = octave_range;

            float freq = get_frequency_for_voltage(value * this->octave_range);
        ...
        virtual const char* getFormattedValue() {
            ...
            float voltage = this->currentDataValue * this->octave_range;
            ...
            snprintf(fmt, 20, "%4uhz [%3s]", ifreq, get_note_name_c(pitch)); //->getCurrentValue());
